...
Publishing atomically copies draft to published and increments `published_version`, which drives the config ETag. Brand assets use UUID references into workspace-scoped `widget_assets`; the private `widget-assets` bucket is exposed only through short-lived signed URLs after MIME, byte, dimension, workspace, and asset-kind checks. The model accepts typed fields and allowlisted fonts/assets only—no arbitrary CSS, JavaScript, or remote URLs. See `docs/WIDGET-STUDIO.md` and ADR-009.
