...
| Widget request replay from an unauthorized origin | High | `Origin` header, when present, must match the embed token's bound `parentOrigin`; mismatch denied with `403` |
...
| **Query-string/fragment secret leakage via tracked URLs** | `current_url` / `landing_url` / `referrer` / page-view URLs are passed through an allowlist sanitizer (origin + path + `utm_*` only; fragment and all other query params dropped) enforced in both shared TypeScript and SQL (`app_private.sanitize_page_url`) before any write — nothing sensitive ever reaches storage or the dashboard |
| **Widget request from a mismatched Origin** | When a browser `Origin` header is present, it must equal the embed token's bound `parentOrigin` (`requestOriginMatchesEmbed`); mismatches are rejected `403 FORBIDDEN` on `session`, `identify`, and `page-view` |
