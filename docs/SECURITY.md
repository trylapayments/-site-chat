...
| Global search | `public.global_search` for authenticated workspace members; viewers get `can_search_notes = false` and empty notes groups; anon/visitors cannot execute; results always filtered by `workspace_id` |
...
Global search (`public.global_search`) follows the same SECURITY DEFINER pattern with empty `search_path`. Authorization is membership via `require_workspace_access`; notes inclusion is gated by role (`viewer` → empty notes group + `can_search_notes = false`, no error-based existence leak). Attachment hits expose filename/mime only (never `storage_key`). Cross-workspace probes return no foreign hits. See `docs/GLOBAL-SEARCH.md`.
